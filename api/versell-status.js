...
export default async function handler(req, res) {
  ...
  try {
    ...
    // A API VersellPay sempre retorna 200, mesmo quando a transação não existe
    // Então precisamos verificar o conteúdo da resposta, não o status HTTP
    
    // Verifica se a resposta tem a estrutura esperada
    if (!data || typeof data !== "object") {
      return json(res, 200, { 
        ok: false,
        error: "Invalid response from VersellPay API",
        status: "UNKNOWN"
      });
    }
    ...
    // Verifica se a transação existe na resposta
    ...
    // VersellPay retorna "processingStatus" - esse é o campo que importa!
    // PAID_OUT = pago | WAITING_FOR_APPROVAL = aguardando
    const status =
      String(tx?.processingStatus || tx?.statusTransaction || "").trim().toUpperCase() || "UNKNOWN";

    // Devolve o essencial pro front
    return json(res, 200, {
      ok: true,
      status, // WAITING_FOR_APPROVAL | PAID_OUT | UNKNOWN | NOT_FOUND
